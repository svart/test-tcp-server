...
async fn server_connection_handler(mut sock: TcpStream) {
    ...
    loop {
        match sock.read_exact(&mut send_buf[..HEADER_SIZE]).await {
            Ok(0) | Err(_) => return,
            Ok(_) => {
                let req_id = u64::from_be_bytes(send_buf[..ONE_HEADER_FIELD].try_into().expect("request should be ok"));
                ...
                while left_to_send > 0 {
                    let fun = if left_to_send < SEND_BUFFER_SIZE {
                        println!("sending {left_to_send} bytes");
                        sock.write_all(&send_buf[..left_to_send])
                    } else {
                        println!("sending {SEND_BUFFER_SIZE} bytes");
